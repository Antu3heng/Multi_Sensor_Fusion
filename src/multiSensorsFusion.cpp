 ...
 */
    MSF::MSF()
    {
        ...
        state_.bw_.setZero();
        state_.imuAcc_.setZero();
        state_.imuGyro_.setZero();
        vio_p_map_.setZero();
 ...
 */
    void MSF::inputIMU(const double &timestamp, const Vector3d &imuAccRaw, const Vector3d &imuGyroRaw)
    {
        if (!isInitialized())
        {
            state_.imuAcc_ = imuAccRaw;
            state_.imuGyro_ = imuGyroRaw;
        ...
        } else
        {
            ...
            double dt = timestamp - last_state_.timestamp_;
            Vector3d acc = 0.5 * (last_state_.imuAcc_ + imuAccRaw) - last_state_.ba_;
            Vector3d gyro = 0.5 * (last_state_.imuGyro_ + imuGyroRaw) - last_state_.bw_;
 ...
 * @param vioCovariance 
 */
    void MSF::inputVIO(const double &timestamp, const Vector3d &vioPosRaw, const Vector3d &vioVelRaw,
                       const Quaterniond &vioRotRaw, const Matrix<double, 9, 9> &vioCovariance)
    {
        if (!isInitialized())
        {
