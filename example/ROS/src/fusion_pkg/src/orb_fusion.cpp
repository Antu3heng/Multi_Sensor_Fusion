...
{
    ros::init(argc, argv, "orb_fusion");
    ros::NodeHandle nh("~");

    if (argc != 2)
    {
        cerr << endl
             << "Usage: rosrun fusion_pkg orb_fusion path_to_config" << endl;
        return 1;
    }
    ...
    ros::Subscriber imu_sub = nh.subscribe("/airsim_node/drone_1/imu/imu", 100, imuCallback);
    ros::Subscriber mapLoc_sub = nh.subscribe("/Rgbd_Inertial/ORB_SLAM3/RGBD_IMU/pose", 10, mapLocCallback);
